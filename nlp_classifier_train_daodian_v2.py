...
if __name__ == '__main__':
    ...
    # read custom data
    data_files = {
        "train": "/home/ma-user/work/MultimodalSimilar/daodian_data_v2/train.pqt",
        "test": "/home/ma-user/work/MultimodalSimilar/daodian_data_v2/test.pqt"}
    ...
    train_dataloader = DataLoader(
        tokenized_datasets["train"], shuffle=False, sampler=sampler ,pin_memory=True, drop_last=True, batch_size=batch_size, collate_fn=data_collator
    )
    ...
    optimizer_fc = AdamW(model.parameters(), lr=1e-3)
    ...
    for epoch in range(num_epochs):
        for step, batch in enumerate(train_dataloader, start=1):
            ...
            # save model
            if global_step % 1000 == 0:
                torch.save(model,'./nlp_model_v3/{}.pt'.format(global_step))
    # model.eval()
    # for batch in test_dataloader:
    #     batch = {k: v.to(device) for k, v in batch.items()}
    #     with torch.no_grad():
