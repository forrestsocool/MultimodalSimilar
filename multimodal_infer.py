...
if __name__ == '__main__':
    ...
    model = torch.load('./multimodal_model/3000.pt', map_location=device)
    model.eval()
    vec_result = []
    spusn_list = []
    ...
    for i, item in pd_multimodal.iterrows():
        try:
            img_tensor, title_tokens = tokenize_function(item)
            title_tensors = data_collator(title_tokens)
            ticks = time.time()
            embedding = model.predict_emb(img_input=img_tensor.unsqueeze(0).to(device),
