...
if __name__ == '__main__':
    ...
    model = NlpClassifier(pretrained_model, 627)
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
    model.to(device)

    # read custom data
    data_files = {
        "train": "/home/ma-user/work/MultimodalSimilar/daodian_data/train.csv",
        "test": "/home/ma-user/work/MultimodalSimilar/daodian_data/test.csv"}
    raw_datasets = load_dataset("csv", data_files=data_files, delimiter=",")
    tokenized_datasets = raw_datasets.map(tokenize_function, batched=True)
    data_collator = DataCollatorWithPadding(tokenizer=tokenizer)
    # post process
    tokenized_datasets = tokenized_datasets.remove_columns(["title", "goods_sku"])
    tokenized_datasets = tokenized_datasets.rename_column("lv2_category_id", "labels")
    ...
    for epoch in range(num_epochs):
        for step, batch in enumerate(train_dataloader, start=1):
            ...
            if global_step % 1000 == 0:
                torch.save(model,'./nlp_model_daodian/{}.pt'.format(global_step))
    # model.eval()
    # for batch in test_dataloader:
    #     batch = {k: v.to(device) for k, v in batch.items()}
    #     with torch.no_grad():
