...
nlp_score_th = 0.4
cv_score_th = 0.4
area_list = [114, 102, 106, 112, 105, 101, 111, 119, 115, 104, 110, 108, 118,
       109, 107, 113, 117]
# area_list = [114]
...
def get_similar_nlp(frxs_product, nlp_model):
    ...
    pd_spusn_proceed['title'] = pd_spusn_proceed.apply(lambda x: gen_title(x.product_name, x.product_title), axis=1)
    ...
    D, I = index.search(arr_result, 13)  # actual search
...
def get_similar_cv(frxs_product, cv_model):
    ...
    for i in range(len(spusn_list)):
        ...
        for similar_index, score in zip(I[i][1:], D[i][1:]):
            ...
            if score > cv_score_th and cate_id==curr_cate:
...
if __name__ == '__main__':
    ...
    for area_id in area_list:
        ...
        for key in area_results:
            ...
            if len(result) > 0:
                result_str = json.dumps(result).replace('[', '').replace(']', '').replace('"', '')
